...
type Props = { data: SiteJson; onChange: (next: SiteJson) => void };
const fieldClass = () => "w-full rounded-md border border-input bg-background px-2 py-1.5 text-xs";

export function SiteWorkProjectsEditor({ data, onChange }: Props) {
  const [skillInput, setSkillInput] = useState("");
  const apply = (fn: (j: SiteJson) => void) => { const next = structuredClone(data); fn(next); onChange(next); };
