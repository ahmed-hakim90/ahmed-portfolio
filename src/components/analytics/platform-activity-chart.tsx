...
export function PlatformActivityChart({ dailyActivity }: Props) {
  ...
  const maxPdf = Math.max(1, ...rows.map((r) => r.pdfDownloads));
    ...
    <div className="space-y-6 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-6">
      ...
      <div>
        ...
        <div
          className="mt-4 flex h-20 items-end gap-1 sm:gap-1.5"
          role="img"
          aria-label="مخطط تحميلات PDF اليومية"
        >
          {rows.map((row) => {
            const pct = Math.max(3, (row.pdfDownloads / maxPdf) * 100);
            return (
              <div
                key={`pdf-${row.date}`}
                className="flex min-w-0 flex-1 flex-col items-stretch justify-end"
              >
                <div
                  className="w-full rounded-sm bg-sky-600/75 motion-reduce:transition-none dark:bg-sky-500/70"
                  style={{ height: `${pct}%` }}
                  title={`${row.date}: ${row.pdfDownloads} تحميل`}
                />
                <span className="mt-1 truncate text-center text-[10px] text-muted-foreground sm:text-[11px]">
                  {row.date.slice(5)}
                </span>
